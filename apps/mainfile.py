...
def seqLengthcalculate(fname):
    ...
    return fn1
'''
def compare_dataframes(gts, ts,seqLength, iou=0.5):
    """Builds accumulator for each sequence."""
    accs = []
    anas = []
    acc,ana=mm.utils_copy.CLEAR_MOT_M(gts,ts,seqLength, 'iou', distth=iou, vflag='')
    accs.append(acc)
    anas.append(ana)
    return accs,anas
'''
...
def parseSequences(seqmap):
    ...
    assert os.path.exists(seqmap), 'Seqmap %s not found.' % seqmap
    fd = io.open(seqmap)
    res = []
...
def main():
    ...
    gtfile = os.path.join(args.groundtruths)
    tsfile = os.path.join(args.tests)
    newseq=Sequencesmaplist(gtfile)
    '''
    for i in newseq:
        print(i)
    '''
    ...
    '''
    seqLength= seqLengthcalculate(gtfile)
    if args.skip > 0 and 'mot' in args.fmt:
            gtfile = generateSkippedGT(gtfile, args.skip, fmt=args.fmt)
    #seqLength=10000
    #seqLength= seqLengthcalculate(gtfile)
    #print(seqLength)
    #seqLength=600
    #print(seqLength)
    #gt=mm.io_copy.loadtxt(args.groundtruths, fmt=args.fmt)
    #ts=mm.io_copy.loadtxt(args.tests, fmt=args.fmt)
    #gt = OrderedDict([(seqs[i], (mm.io_copy.loadtxt(f, fmt=args.fmt), os.path.join(args.groundtruths, seqs[i], 'seqinfo.ini'))) for i, f in enumerate(gtfiles)])
    #ts = OrderedDict([(seqs[i], mm.io_copy.loadtxt(f, fmt=args.fmt)) for i, f in enumerate(tsfiles)])

    mh = mm.metrics.create()

    st = time.time()
    #acc,ana=mm.utils.CLEAR_MOT_M(gts,ts,~/Desktop/MOT16Labels/train/MOT16-02/seqinfo.ini , 'iou', distth=iou, vflag='')

    accs, analysis=mm.io.load_motchallenge(args.groundtruths,args.tests,seqLength,newseq)

    logging.info('adding frames: %.3f seconds.', time.time() - st)

    logging.info('Running metrics')

    summary = mh.compute_many(accs, anas=analysis, names=newseq, metrics=[ 'mota', 'motp'], generate_overall=True)
