...
def load_motchallenge(fname1,fname2,seqLength, sequencename, **kwargs):
    ...
    df1 = pd.read_csv(
        ...
        header=None,
        names=['Filename','FrameId', 'Id', 'X', 'Y', 'Width', 'Height', 'Confidence', 'ClassId', 'Visibility', 'unused'],
        engine='python'
    )
    df2 = pd.read_csv(
        fname2,
    ...
    grouped2=df2.groupby('Filename')
    for i in sequencename:
        df_new1 = grouped1.get_group(i)
        
        # Remove all rows without sufficient confidence
        df_new1 = df_new1[df_new1['Confidence'] >= min_confidence]
        df_new2 = grouped2.get_group(i)
        
        # Remove all rows without sufficient confidence
        df_new2 =df_new2[df_new2['Confidence'] >= min_confidence]
        del df_new1['unused']
        del df_new1['Filename']
        del df_new2['unused']
        del df_new2['Filename']
        #print(df_new2)
        #print(df_new1)
        acc,ana=mm.utils.CLEAR_MOT_M(df_new1,df_new2,seqLength, 'iou', distth=0.5, vflag='')
        accs.append(acc)
        anas.append(ana)
